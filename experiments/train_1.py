...
train_dataset = train_dataset.map(lambda x: df.add_gauss_noise_to_image(x, alphas_cumprod, T))

# build test dataset
x_test = tf.data.Dataset.from_tensor_slices(x_test)
x_test = x_test.batch(batch_size).prefetch(2)
x_test = x_test.map(lambda x: df.add_gauss_noise_to_image(x, alphas_cumprod, T))

#checkpoit
checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(
    os.path.join(MODEL_DIR, 'model-{epoch}.h5'),
